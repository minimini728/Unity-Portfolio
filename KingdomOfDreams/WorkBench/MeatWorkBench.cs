...
public class MeatWorkBench : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        this.amountItem2 = InfoManager.instance.IngredientInfos.Find(x => x.id == 2004).amount;
    ...
    public void Init()
    {
        Debug.Log("<color=blue>정육점 조합대 Init</color>");
    }
    private void Make(int num)
    {
        ...
        {
            //아이템 얻기
            EventDispatcher.instance.SendEvent<int>((int)LHMEventType.eEventType.COMBINE_INGREDIENT, 2007);
        }

        //아이템 차감
        InfoManager.instance.IngredientInfos.Find(x => x.id == 2000).amount -= numItem1;
        InfoManager.instance.IngredientInfos.Find(x => x.id == 2004).amount -= numItem2;
        //수 초기화
        this.numWant = 0;
    }
